  ...
  { TRpcThread }
    ...
    TRpc = class
      ...
      public
        ...
        function SendRequest(req: TJSONObject; ReturnArguments: boolean = True; ATimeOut: integer =
                             -1): TJSONObject;
        function RequestInfo(TorrentId: integer; const Fields: array of const; const ExtraFields:
...
begin
  ...
  if RpcObj.RPCVersion >= 4 then
    ...
    dir :='';
  MainForm.FillFilesList(t.Integers['id'], t.Arrays['files'], t.Arrays['priorities'], t.Arrays[
                         'wanted'], dir);
end;
...
args :=FRpc.RequestInfo(0, ['id', 'name', 'status', 'errorString', 'announceResponse',
       'recheckProgress',
...
var
  ...
  t: TJSONArray;
begin
  args :=FRpc.RequestInfo(TorrentId, ['totalSize', 'sizeWhenDone', 'leftUntilDone', 'pieceCount',
         'pieceSize', 'haveValid',
         ...
         'maxConnectedPeers', 'nextAnnounceTime', 'dateCreated', 'creator', 'eta',
         'peersSendingToUs',
...
function TRpc.SendRequest(req: TJSONObject; ReturnArguments: boolean; ATimeOut: integer):
                                                                                         TJSONObject
;
      ...
      if Http.ResultCode = 409 then
        begin
          ...
          for j:=0 to Http.Headers.Count - 1 do
            if Pos('x-transmission-session-id:', AnsiLowerCase(Http.Headers[j])) > 0 then
              begin
...
function TRpc.RequestInfo(TorrentId: integer; const Fields: array of const; const ExtraFields: array

...
&Add torrent=Προσθήκη torrent(&A)
...
Queue=Ουρά
Torrents that are idle for N minutes aren't counted toward the Download queue or Upload queue=Τα ανενεργά torrents για N λεπτά δεν υπολογίζονται στην Ουρά Λήψεων ή την Ουρά Αποστολών
...
Always auto-reconnect=Πάντα αυτόματη επανασύνδεση
Application Options (Ctrl+F9)=Επιλογές εφαρμογής (Ctrl+F9)
Big Toolbar=Μεγάλη γραμμή εργαλείων
Client Certificate=Πιστοποιητικό πελάτη
Connection refused=Η σύνδεση απορρίφθηκε
Connection timed out=Η σύνδεση έληξε
Copy Magnet Link(s)=Αντιγραφή συνδέσμου(ων) Magnet
Default=Προεπιλογή
Delete destination folder=Διαγραφή φακέλου προορισμού
Development site=Ιστότοπος ανάπτυξης
Done: %s=Ολοκληρώθηκε: %s
Export settings=Εξαγωγή ρυθμίσεων
Host not found=Ο υπολογιστής δεν βρέθηκε
Import settings=Εισαγωγή ρυθμίσεων
Left->Right=Αριστερά->Δεξιά
Magnet Link=Σύνδεσμος Magnet
Menu=Μενού
Other Winsock error=Άλλο σφάλμα Winsock
Private Key=Ιδιωτικό κλειδί
Remaining: %s=Απομένουν: %s
Right->Left=Δεξιά->Αριστερά
Right->Left (No Align)=Δεξιά->Αριστερά (χωρίς στοίχιση)
Right->Left (Reading Only)=Δεξιά->Αριστερά (μόνο ανάγνωση)
Selected: %s=Επιλεγμένα: %s
Size left=Μέγεθος που απομένει
Status bar - Sizes=Γραμμή κατάστασης - Μεγέθη
Template:  =Πρότυπο:
The maximum history of directories for storage of torrents=Το μέγιστο ιστορικό καταλόγων για αποθήκευση torrent

...
&Add torrent=Lägg till torrent(&A)
Added on=Lades till
...
Download speeds (KB/s)=Nedladdningshastigheter (KB/s)
...
A new version of %s is available.~Your current version: %s~The new version: %s~~Do you wish to open the Downloads web page?=En ny version av %s är tillgänglig.~Din nuvarande version: %s~Den nya versionen: %s~~Vill du öppna nedladdningssidan?
Advanced=Avancerad

...
Queue=Fronta
Torrents that are idle for N minutes aren't counted toward the Download queue or Upload queue=Torrenty nečinné alespoň N minut se nezpočítavají do Stahovací nebo Odesílací fronty
...
%dmo=%dměs
%dy=%dr
Access violation=Porušení přístupu
Always auto-reconnect=Vždy automaticky znovu připojit
Application Options (Ctrl+F9)=Možnosti aplikace (Ctrl+F9)
Big Toolbar=Velký panel nástrojů
Client Certificate=Klientský certifikát
Connection refused=Připojení odmítnuto
Connection timed out=Vypršel časový limit připojení
Copy Magnet Link(s)=Kopírovat Magnet odkaz(y)
Default=Výchozí
Delete destination folder=Smazat cílovou složku
Development site=Vývojářský web
Done: %s=Hotovo: %s
Export settings=Exportovat nastavení
Host not found=Hostitel nenalezen
Import settings=Importovat nastavení
Left->Right=Zleva doprava
Magnet Link=Magnet odkaz
Menu=Nabídka
Other Winsock error=Jiná chyba Winsock
Private Key=Soukromý klíč
Remaining: %s=Zbývá: %s
Right->Left=Zprava doleva
Right->Left (No Align)=Zprava doleva (bez zarovnání)
Right->Left (Reading Only)=Zprava doleva (pouze čtení)
Selected: %s=Vybráno: %s
Size left=Zbývající velikost
Status bar - Sizes=Stavový řádek - Velikosti
Template:  =Šablona:
The maximum history of directories for storage of torrents=Maximální historie složek pro ukládání torrentů

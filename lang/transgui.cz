...
Connect to daemon=Připojit k démonovi
connected=připojeno
...
D: %s/s=D: %s/s
Delete host settings=Smazat nastavení hosta
...
Download dir=Adresář pro stahování
...
Password=Heslo
Paths=Cesty
Pick random port on daemon launch=Vybrat náhodný port po staru démona
...
Show parameters window when adding a new torrent=Zobrazit okno vlastností při přidávání torrentu
Show=Zobrazit
Size=Velikost
skip=přeskočit
...
Unable to find path mapping.~Use program's options to setup path mappings=Není možné najít mapování cest.~Použijte možnosti programu k nastavení mapování cest
...
Use proxy server=Použít proxy server
...
Working=Běží
Yes to &All=Ano &Všem
You should restart the application to apply changes=Aby se projevily změny je třeba aplikaci restartovat
No tracker=Žádný tracker
%s downloaded=%s staženo
%s of %s downloaded=%s z %s staženo
Tracker did not respond=tracker neodpovídá
...
Connection options=Nastavení spojení
Days=Dny
Delete=Smazat
Disconnect=Odpojit
...
Proxy=Proxy
Refresh interval when minimized=Interval obnovení při minimalizaci
...
The blocklist URL was not specified=Není nastavena URL blocklistu
...
User interface scaling=Zvětšní uživatelského rozhraní
View=Zobrazit

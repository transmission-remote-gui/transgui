...
Consider active torrents as stalled when idle for=Pomiń aktywne torrenty, które utknęły w martwym punkcie, gdy bezczynne
...
Queue=Kolejka
Torrents that are idle for N minuets aren't counted toward the Download queue or Upload queue=Torrenty, które są nieaktywne przez N minut są pominięte z kolejki pobierania lub wysyłania
Tracker grouping=Grupowanie trackerów
Upload queue size=Rozmiar kolejki wysyłania
User interface scaling=Skalowanie interfejsu użytkownika
View=Widok

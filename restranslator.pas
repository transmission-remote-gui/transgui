...
type
  ...
  TResTranslator = class;
    ...
    TTranslateStringEvent = procedure (Sender: TResTranslator; const ResourceName: AnsiString; var
                                       Accept: boolean);
  ...
  { TResTranslator }
        ...
        TResTranslator = class(TAbstractTranslator)
          private
            ...
            function InternalTranslateString(const Value: AnsiString; IsExternal: boolean = False):
                                                                                          AnsiString
            ;
          public
            ...
            destructor Destroy;
            override;
            procedure TranslateStringProperty(Sender: TObject; const Instance: TPersistent; PropInfo
            ...
            property IgnoreDelimiters: TWordDelimitersOptions Read FIgnoreDelimiters Write
                                       SetIgnoreDelimiters;
            ...
            property OnTranslateString: TTranslateStringEvent Read FOnTranslateString Write
                                        SetOnTranslateString;
        end;

        function LoadTranslationFile(const TranslationFile: AnsiString; const OnTranslate:
                                     TTranslateStringEvent = nil): AnsiString;
        ...
        overload;
        function LoadDefaultTranslationFile(const OnTranslate: TTranslateStringEvent = nil):
                                                                                           TFileName
        ;
        function LoadDefaultTranslationFile(const TranslationFilesPath: AnsiString; const
                                            OnTranslate: TTranslateStringEvent = nil): TFileName;
        function LoadLanguageTranslation(const Language: AnsiString; const OnTranslate:
                                         TTranslateStringEvent = nil): TFileName;
        function LoadLanguageTranslation(const Language, TranslationFilesPath: AnsiString; const
                                         OnTranslate: TTranslateStringEvent = nil): TFileName;
        ...
        function GetTranslationFileName(const Language: AnsiString; AvailableTranslations:
...
{ procedures and functions }
    ...
    begin
      ...
      if FindFirstUTF8(IncludeTrailingPathDelimiter(SearchPath) + '*', faArchive or faReadOnly, Sr)
         = 0 then
        with Result do
          begin
            NameValueSeparator := '=';
    ...
    begin
      ...
      with Sl do
        for i := 0 to Count - 1 do
          SupplementTranslationFile(IncludeTrailingPathDelimiter(TranslationFilesPath) +
          ValueFromIndex[i]);
...
function LoadDefaultTranslationFile(const TranslationFilesPath: AnsiString; const OnTranslate:
...
function LoadLanguageTranslation(const Language, TranslationFilesPath: AnsiString; const OnTranslate
                                 : TTranslateStringEvent): TFileName;
...
function GetTranslationFileName(const Language: AnsiString; AvailableTranslations: TStringList):
                                                                                          AnsiString
;
    ...
    with AvailableTranslations do
      for i := 0 to Count - 1 do
        begin
          ...
          if AnsiSameText(AnsiDequotedStr(Language, QuoteChar), AnsiDequotedStr(aName, QuoteChar))
            then
            begin
  ...
  begin
    ...
    if j > 0 then
      begin
        ...
        Strings[i] :=NormaliseQuotedStr(Copy(s, 1, j)) + NameValueSeparator + NormaliseQuotedStr(
                     Copy(s, k, MaxInt));
      end;
  end;
end;
      ...
      begin
        ...
        if tsoExternal in Options[i] then
          begin
            j :=ScanQuotSep(PChar(s));
            if j > 0 then
              s :=NormaliseQuotedStr(Copy(s, 1, j)) + esep + NormaliseQuotedStr(Copy(s, j + 2,
                  MaxInt));
          end;
        if s <> '' then
    ...
    begin
      ...
      if FileExistsUTF8(FTranslationFile) then
        begin
          ...
          FTranslationLanguage := AnsiDequotedStr(CValues[AnsiQuotedStr(sLanguageIDName, QuoteChar)]
                                 , QuoteChar);
        end;
    end;
end;
...
function TResTranslator.InternalTranslateString(const Value: AnsiString; IsExternal: boolean):
                                                                                          AnsiString
;
      ...
      with FStrResLst do
        begin
          ...
          if (s = '') then
            ...
            begin
              Result := StringReplace(Result, ClearValue, AnsiDequotedStr(s, QuoteChar), [
                       rfReplaceAll]);
...
procedure TResTranslator.TranslateStringProperty(Sender: TObject; const Instance: TPersistent;

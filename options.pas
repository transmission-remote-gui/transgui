...
uses
Classes, SysUtils, FileUtil, LResources, Forms, Controls, Graphics, Dialogs, ComCtrls, StdCtrls,
Spin, Buttons, ButtonPanel, BaseForm,
ConnOptions;
...
type
  ...
  TOptionsForm = class(TBaseForm)
    ...
    procedure cbLanguageMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y:
                                  Integer);
    ...
    if reg.OpenKeyReadOnly('Software\Classes\.torrent') then
      begin
        if reg.ReadString('') = AppName then
          begin
            reg.CloseKey;
            if reg.OpenKeyReadOnly(Format('Software\Classes\%s\shell\open\command', [AppName])) then
              begin
                s :=reg.ReadString('');
...
procedure TOptionsForm.cbLanguageMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState
                                           ; X, Y: Integer);
begin
        ...
        begin
          if reg.OpenKey('Software\Classes\.torrent', True) then
            begin
              ...
              reg.CloseKey;
              if reg.OpenKey(Format('Software\Classes\%s\DefaultIcon', [AppName]), True) then
                begin
                  reg.WriteString('', Format('"%s",0', [ParamStr(0)]));
                  reg.CloseKey;
                  if reg.OpenKey(Format('Software\Classes\%s\shell\open\command', [AppName]), True)
                    then
                    begin
                ...
                begin
                  ...
                  if reg.OpenKey('Software\Classes\Magnet\shell', True) then
                    begin
                      ...
                      reg.CloseKey;
                      if reg.OpenKey('Software\Classes\Magnet\shell\open\command', True) then
                        begin
                          reg.WriteString('', Format('"%s" "%%1"', [ParamStr(0)]));
                          reg.CloseKey;
                        end;

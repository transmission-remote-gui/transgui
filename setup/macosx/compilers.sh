...
if [ ! -x "$(command -v fpc 2>&1)" ]; then
  wget "https://downloads.sourceforge.net/project/lazarus/Lazarus%20Mac%20OS%20X%20i386/Lazarus%201.6.4/$fpc.dmg?r=&ts=$(date +%s)" -O $fpc.dmg
...
if [ ! -x "$(command -v lazbuild 2>&1)" ]; then
  wget "https://downloads.sourceforge.net/project/lazarus/Lazarus%20Mac%20OS%20X%20i386/Lazarus%201.6.4/$lazarus.dmg?r=&ts=$(date +%s)" -O "$lazarus.dmg"

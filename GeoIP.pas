...
type
  ...
  private
    ...
    FInitResult: TGeoIPResult;
    FDatabaseCorrupt: Boolean;
    FDataEnd: Int64;
  ...
  public
    ...
    property DatabaseCorrupt: Boolean read FDatabaseCorrupt;
...
begin
  if FInitResult <> GEOIP_SUCCESS then
  begin
    Result := FInitResult;
    Exit;
  end;
...
begin
  ...
  FInitResult := GEOIP_SUCCESS;
  FDatabaseCorrupt := False;
  ...
  begin
    ...
    begin
      ...
      if MarkerOffset + 4 > FileSize then
      begin
        FInitResult := GEOIP_ERROR_IO;
        FDatabaseCorrupt := True;
        SetLength(FDatabaseSegments, 0);
      ...
      if DatabaseTypeValue < Ord(Low(TGeoIPDBTypes)) then
      begin
        FInitResult := GEOIP_ERROR_IO;
        FDatabaseCorrupt := True;
        SetLength(FDatabaseSegments, 0);
      ...
      if DatabaseTypeValue > Ord(High(TGeoIPDBTypes)) then
      begin
        FInitResult := GEOIP_ERROR_DBTYPE;
        SetLength(FDatabaseSegments, 0);
        Exit;
      end;
      ...
      begin
        ...
        if MarkerOffset + 4 + SEGMENT_RECORD_LENGTH > FileSize then
        begin
          FInitResult := GEOIP_ERROR_IO;
          FDatabaseCorrupt := True;
          SetLength(FDatabaseSegments, 0);

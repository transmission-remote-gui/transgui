  ...
  { TFilesTree }
    ...
    TFilesTree = class(TComponent)
      private
        ...
        procedure DoCellAttributes(Sender: TVarGrid; ACol, ARow, ADataCol: integer; AState:
                                   TGridDrawState; var CellAttribs: TCellAttributes);
        procedure DoCheckBoxClick(Sender: TVarGrid; ACol, ARow, ADataCol: integer);
        procedure DoDrawCell(Sender: TVarGrid; ACol, ARow, ADataCol: integer; AState: TGridDrawState
                             ; const R: TRect; var ADefaultDrawing: boolean);
        ...
        function DoCompareVarRows(Sender: TVarList; Row1, Row2: PVariant; DescendingSort: boolean):
                                                                                             integer
        ;
...
begin
  FFiles.RowOptions[ARow] :=(FFiles.RowOptions[ARow] and not roChecked) or (integer(AValue) shl
                            roCheckedShift);
end;
...
function TFilesTree.DoCompareVarRows(Sender: TVarList; Row1, Row2: PVariant; DescendingSort: boolean
): integer;
begin
  if FGrid.SortColumn <> idxFileName then
    begin
      Result :=(integer(VarIsEmpty(Sender.GetRowItem(Row1, idxFileId))) and 1) - (integer(VarIsEmpty
               (Sender.GetRowItem(Row2, idxFileId))) and 1);
      exit;
    end;

  Result :=CompareVariants(Sender.GetRowItem(Row1, idxFileFullPath), Sender.GetRowItem(Row2,
           idxFileFullPath));
  if Result <> 0 then
    exit;
  Result :=(integer(VarIsEmpty(Sender.GetRowItem(Row2, idxFileId))) and 1) - (integer(VarIsEmpty(
           Sender.GetRowItem(Row1, idxFileId))) and 1);
  if Result <> 0 then
    exit;
...
procedure TFilesTree.DoCellAttributes(Sender: TVarGrid; ACol, ARow, ADataCol: integer; AState:
                                      TGridDrawState; var CellAttribs: TCellAttributes);
      ...
      case ADataCol of
        0:
          ...
          //        Text:=(Sender.Items[idxFileFullPath, ARow]) + ' (' + Text + ')';// Lazarus 1.4.4
             ...
             if IsPlain then
               begin
                 Text :=Copy(UTF8Encode(widestring(Sender.Items[idxFileFullPath, ARow])),
...
procedure TFilesTree.DoDrawCell(Sender: TVarGrid; ACol, ARow, ADataCol: integer; AState:
  ...
  for i:=ARow to Sender.Items.Count - 1 do
    begin
      ...
      if VarIsEmpty(v) or VarIsNull(v) or (IsPlain and IsFolder(i)) then
        continue;
      if Pos(s, Trim(LazUTF8.UTF8UpperCase((widestring(v))))) > 0 then
        begin
          ARow :=i;
...
function  TAddTorrentForm.IsFileTemplate(filename:string; cntE : integer; e: array of string):
                                                                                             boolean
;

...
begin
  ...
  Result:=LazFileUtils.DeleteFileUTF8(FileName) or
    not FileExistsUTF8(FileName);
end;

function OpenIPCRecoveryFile: System.THandle;
begin
  Result:=utils.FileOpenUTF8(FIPCFileName + '.recovery',
    fmOpenReadWrite or fmShareExclusive);
  if (Result <> System.THandle(feInvalidHandle)) and
    not RestrictPrivateFileRights(Result, FIPCFileName + '.recovery') then begin
    FileClose(Result);
    Result:=System.THandle(feInvalidHandle);
  end;
end;
...
begin
  Result:=False;
{$ifndef windows}
  PreserveMode:=fpStat(PChar(LazUTF8.UTF8ToSys(TargetFileName)), FileInfo) = 0;
{$endif windows}
{$ifdef windows}
...
begin
  ...
  if Result then
    Result:=ReplaceResolvedFileContents(TargetFileName, Content);
end;
...
{$endif windows}
  Result:=ReplaceResolvedFileContents(FileName, Content);
end;
...
begin
  ...
  if h = System.THandle(feInvalidHandle) then
    exit;
  try
    Result:=RestrictPrivateFileRights(h, FRunFileName) and
      WriteFileAll(h, Identity[1], Length(Identity));
  finally
    FileClose(h);
  end;
  if not Result then
...
begin
{$ifdef windows}
  Attributes:=GetFileAttributesW(PWideChar(UTF8Decode(FRunFileName)));
  Result:=False;
  if (Attributes = INVALID_FILE_ATTRIBUTES) or
    ((Attributes and FILE_ATTRIBUTE_REPARSE_POINT) = 0) then
    exit;
  Result:=True;
  h:=CreateFileW(PWideChar(UTF8Decode(FRunFileName)),
    FILE_READ_ATTRIBUTES or FileReadEAFlag,
...
{$else}
  Result:=(fpLStat(PChar(LazUTF8.UTF8ToSys(FRunFileName)), FileInfo) = 0) and
...
begin
  ...
  if IPCData = '' then begin
    if PreserveEmpty then begin
      if (StaleData <> '') and
        not ReplaceFileContents(FIPCFileName, IPCData) then
        exit;
    end
    else if not LazFileUtils.DeleteFileUTF8(FIPCFileName) and
      FileExistsUTF8(FIPCFileName) then
      exit;
  end
...
begin
  ...
  if DeleteFile then begin
    Result:=LazFileUtils.DeleteFileUTF8(FIPCFileName) or
...
begin
  Result:=QueueTorrentFileWithReceipt(FileName, QueueReceipt);
end;
...
var
  ...
  QueuedActivation, RunningInstance, PreservedStaleQueue,
    ...
    IPCFileMissing, OversizedRunFile, QueueAcknowledged: boolean;
  ...
  begin
    ...
    for ArgumentIndex:=0 to High(FailedStartupArguments) do
      if not QueueTorrentFileWithReceipt(
        FailedStartupArguments[ArgumentIndex], QueueReceipt) then begin
        Result:=False;
...
{$endif linux}
  ...
  for i:=1 to ParamCount do begin
    ...
    if IsProtocolSupported(s) or FileExistsUTF8(s) then
      if QueueTorrentFileWithReceipt(s, QueueReceipt) then begin
        QueuedIPCReceipt:=QueueReceipt;
  ...
  try
    ...
    if RunningInstance then begin
      ...
      if not EnsureIPCFile then begin
        if FStartupQueueError = '' then begin
          FStartupQueueError:=FIPCFileName;
          FStartupFileError:=True;
        end;
      ...
      if not QueuedActivation and (Length(FailedStartupArguments) = 0) then begin
...
begin
  ...
  if SaveDialog1.Execute then begin
    ...
    if LazFileUtils.CompareFilenames(
      ...
      exit;
    if not ReplaceResolvedFileContents(ResolvedS, FileData) then
      ShowFileAccessError(s);
  end;
end;

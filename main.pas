...
const
  ...
  sTorrentVerification = 'Torrent verification may take a long time.' + LineEnding +
                         'Are you sure to start verification of torrent ''%s''?';
  sTorrentsVerification = 'Torrents verification may take a long time.' + LineEnding +
  ...
  sGeoIPConfirm = 'Geo IP database is needed to resolve country by IP address.' + LineEnding +
                  'Download this database now?';
  sFlagArchiveConfirm = 'Flag images archive is needed to display country flags.' + LineEnding +
                        'Download this archive now?';
  sInSwarm = 'in swarm';
  ...
  sTorrents = 'Torrents';
  sBlocklistUpdateComplete = 'The block list has been updated successfully.' + LineEnding +
                             'The list entries count: %d.';
...
type
  ...
  TMainForm = class(TBaseForm)
    ...
    procedure gTorrentsCellAttributes(Sender: TVarGrid; ACol, ARow, ADataCol: integer; AState:
                                      TGridDrawState; var CellAttribs: TCellAttributes);
    procedure gTorrentsClick(Sender: TObject);
    procedure gTorrentsDblClick(Sender: TObject);
    procedure gTorrentsDrawCell(Sender: TVarGrid; ACol, ARow, ADataCol: integer; AState:
                                TGridDrawState; const R: TRect; var ADefaultDrawing: boolean);
    ...
    procedure lvFilterCellAttributes(Sender: TVarGrid; ACol, ARow, ADataCol: integer; AState:
                                     TGridDrawState; var CellAttribs: TCellAttributes);
    procedure lvFilterClick(Sender: TObject);
    procedure lvFilterDrawCell(Sender: TVarGrid; ACol, ARow, ADataCol: integer; AState:
                               TGridDrawState; const R: TRect; var ADefaultDrawing: boolean);
    procedure lvPeersCellAttributes(Sender: TVarGrid; ACol, ARow, ADataCol: integer; AState:
                                    TGridDrawState; var CellAttribs: TCellAttributes);
    procedure lvTrackersCellAttributes(Sender: TVarGrid; ACol, ARow, ADataCol: integer; AState:
                                       TGridDrawState; var CellAttribs: TCellAttributes);
    ...
    procedure StatusBarMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y:
                                 Integer);
...
{$endif LCLcarbon}
      ...
      function ExecRemoteFile(const FileName: string; SelectFile: boolean; Userdef: boolean= false):
                                                                                             boolean
      ;
    ...
    public
      ...
      procedure FillFilesList(ATorrentId: integer; list, priorities, wanted: TJSONArray; const
                              DownloadDir: WideString);
      ...
      function TorrentAction(const TorrentIds: variant; const AAction: string; args: TJSONObject =
                             nil): boolean;
      function SetFilePriority(TorrentId: integer; const Files: array of integer; const APriority:
                               string): boolean;
...
procedure DrawProgressCell(Sender: TVarGrid; ACol, ARow, ADataCol: integer; AState: TGridDrawState;
...
const
  ...
  TorrentFieldsMap: array[idxName..idxPrivate] of string =
                                                           ('', 'totalSize', '', 'status',
                                                            ...
                                                            , 'addedDate', 'doneDate',
                                                            'activityDate', '', 'bandwidthPriority',
                                                            '', '', 'queuePosition',
                                                            'secondsSeeding', 'leftUntilDone',
                                                            'isPrivate');
...
var
  TR_STATUS_STOPPED, TR_STATUS_CHECK_WAIT, TR_STATUS_CHECK, TR_STATUS_DOWNLOAD_WAIT,
  TR_STATUS_DOWNLOAD, TR_STATUS_SEED_WAIT, TR_STATUS_SEED: integer;


  {$ifdef windows}
function WndCallback(Ahwnd: HWND; uMsg: UINT; wParam: WParam; lParam: LParam): LRESULT;
stdcall;
begin
...
const
  IgnoreUnits: array[0..12] of string =
                                        ('fpjson','jsonparser','jsonscanner','lclstrconsts','math',
                                         'rtlconsts','sysconst','variants','zbase','zipper',
                                         'zstream',
                                         'xmlcfg', 'registry');
  ...
  if Ini.ReadBool('Translation', 'TranslateForm', True) = False then
    ...
    begin
      ...
      if FTranslationFileName <> '-' then
        if (FTranslationFileName = '') or not IsTranslationFileValid(DefaultLangDir +
           FTranslationFileName) then
          FTranslationLanguage := LoadDefaultTranslationFile(@OnTranslate)
      else
        FTranslationLanguage := LoadTranslationFile(DefaultLangDir + FTranslationFileName, @
                               OnTranslate);
...
begin
  ...
  with FBmp.Canvas do
    begin
      ...
      if FImages <> nil then
        FImages.Draw(FBmp.Canvas, (Self.Width - FImages.Width) div 2, (Self.Height - FImages.Height)
        div 2, ImageIndex);
    end;
...
if Ini.ReadBool('MainForm', 'StatusBar', acStatusBar.Checked) <> acStatusBar.Checked then
  acStatusBar.Execute;
if Ini.ReadBool('MainForm', 'StatusBarSizes', acStatusBarSizes.Checked) <> acStatusBarSizes.Checked
  then
  ...
  begin
    ...
    for i := 1 to j-1 do
      begin
        ...
        try
          if ExtractIconEx(PChar(Ini.ReadString('UserMenu','ExeName'+IntToStr(i),'')), 0, LargeIco,
  ...
  if (SL.Text = '') or (SL.Count <> ActionList.ActionCount) then
    begin
      for i := 0 to ActionList.ActionCount-1 do
        Ini.WriteString('Shortcuts',StringReplace(ActionList.Actions[i].Name,'ac','',[]),
        ShortcutToText(TAction(ActionList.Actions[i]).ShortCut));
      if (i<SL.Count-1) and (SL.Text <> '') and (ActionList.ActionByName(SL.Names[i]) = nil) then
        Ini.WriteString('Shortcuts',StringReplace(ActionList.Actions[i].Name,'ac','',[]),
        ShortcutToText(TAction(ActionList.Actions[i]).ShortCut));
    end
  else
    for i := 0 to SL.Count - 1 do
      try
        TAction(ActionList.ActionbyName('ac'+SL.Names[i])).ShortCut := TextToShortcut(SL.
                                                                      ValueFromIndex[i]);
      except
end;
finally
...
begin
  if not FMainFormShown then
    begin
      FMainFormShown :=True;
      VSplitter.SetSplitterPosition(Ini.ReadInteger('MainForm', 'VSplitter', VSplitter.
                                    GetSplitterPosition));
      HSplitter.SetSplitterPosition(Ini.ReadInteger('MainForm', 'HSplitter', HSplitter.
                                    GetSplitterPosition));
      MakeFullyVisible;
    end;
...
begin
  ...
  if MessageDlg('', Format(SRemoveTracker, [UTF8Encode(widestring(lvTrackers.Items[
     idxTrackersListName, lvTrackers.Row]))]), mtConfirmation, mbYesNo, 0, mbNo) <> mrYes then exit;
  AppBusy;
  Self.Update;
    ...
    try
      ...
      if args = nil then
        ...
        begin
          ...
          t :=Now;
          with gTorrents do
            while not ok and not Application.Terminated and (Now - t < 20/SecsPerDay) do
              begin
                ...
                for i:=0 to Items.Count - 1 do
                  if RowSelected[i] then
                    begin
                      if VarIsEmpty(Items[idxPath, i]) or (AnsiCompareText(UTF8Encode(widestring(
                         Items[idxPath, i])), edTorrentDir.Text) <> 0) then
                        begin
                          ok :=False;
                          break;
                        end;
...
begin
  ...
  if (Sender is TMenuItem) and (TMenuItem(Sender).Tag > 999) then
    begin
      UserDef := true;
        {$ifdef windows}
      FUserDefinedMenuEx    := Ini.ReadString('UserMenu','ExeName'+IntToStr(TMenuItem(Sender).Tag-
                              1000),'');
      FUserDefinedMenuParam := Ini.ReadString('UserMenu','Params'+IntToStr(TMenuItem(Sender).Tag-
                              1000),'');
        {$endif windows}
    end
  else UserDef := false;
...
begin
  ...
  if MessageDlg(s + LineEnding + LineEnding + SUpdateTrackers, mtConfirmation, [mbYes, mbNo], 0) <>
     mrYes then
    exit;
  Application.ProcessMessages;
      ...
      try
        if IsProtocolSupported(FileName) then
          begin
            ...
            if not DownloadFile(FileName, ExtractFilePath(tfn), ExtractFileName(tfn),
               SDownloadingTorrent) then
              exit;
          end
        else
          tfn :=FileName;
...
if (FWatchDownloading) and (FWatchDestinationFolder <> '') then cbDestFolder.Text :=
...
try
  ...
  if (RpcObj.RPCVersion >= 7) and (lvFiles.Items.Count = 0) and (t.Objects[0].Floats[
     'metadataPercentComplete'] <> 1.0) then
    begin
      // Magnet link
      gbContents.Hide;
...
id :=0;
if (Ini.ReadBool('Interface', 'DeleteTorrentFile', False) and not IsProtocolSupported(FileName)) or
   (FWatchDownloading) then
...
{$ifndef CPUARM}
  TrayIcon.Visible :=not IsUnity and
                     ...
                     ((WindowState = wsMinimized) and Ini.ReadBool('Interface', 'TrayMinimize', True
                     ) ) or
...
Procedure TMainForm.DoCreateOutZipStream(Sender : TObject; var AStream : TStream; AItem :
                                         TFullZipFileEntry);
begin
    ...
    if not FileExistsUTF8(FFlagsPath + ImageName) then
      begin
        ...
        try
          ...
          MessageDlg(sUnableExtractFlag + LineEnding + Exception(ExceptObject).Message, mtError, [
          mbOK], 0);
          exit;
      end;
...
begin
  Try
    ...
    DecodeDateTime(Sign(Anow-Athen)*(Anow-AThen),discard,Discard,Discard,Hours,Minutes,Seconds,
    Discard);
    if Years > 0 then
      begin
          ...
          try
            ...
            if RpcObj.RPCVersion >= 5 then
              begin
                ...
                edAltTimeBegin.Text :=FormatDateTime('hh:nn', args.Integers['alt-speed-time-begin']/
                                      MinsPerDay);
                edAltTimeEnd.Text :=FormatDateTime('hh:nn', args.Integers['alt-speed-time-end']/
                                    MinsPerDay);
                j :=args.Integers['alt-speed-time-day'];
                for i:=1 to 7 do
                  begin
                    TCheckBox(gbAltSpeed.FindChildControl(Format('cbDay%d', [i]))).Checked :=
                                                                                            LongBool
                                                                                             (j and
                                                                                             1);
                    j :=j shr 1;
          ...
          if cbAutoAlt.Checked then
            begin
              ...
              for i:=7 downto 1 do
                begin
                  j :=j shl 1;
                  j :=j or (integer(TCheckBox(gbAltSpeed.FindChildControl(Format('cbDay%d', [i]))).
                      Checked) and 1);
                end;
      ...
      case ADataCol of
        idxAddedOn, idxCompletedOn, idxLastActive:
                                                   begin
                                                     ...
                                                     gTorrents.Hint := TorrentDateTimeToString(
                                                                      gTorrents.Items[ADataCol, FRow
                                                                      -1],not(FFromNow));
                                                   end
                                                   else gTorrents.Hint :='';
      end;
  ...
  with TTorrPropsForm.Create(Self) do
    try
      ...
      args :=RpcObj.RequestInfo(id, ['downloadLimit', 'downloadLimitMode', 'downloadLimited',
             ...
             'name', 'maxConnectedPeers', 'seedRatioMode', 'seedRatioLimit', 'seedIdleLimit',
             'seedIdleMode', 'trackers']);
      if args = nil then
        begin
          CheckStatus(False);
          exit;
          ...
          begin
            ...
            for i:=0 to Trackers.Count - 1 do
              begin
                ...
                trlist.AddObject(UTF8Decode(tr.Strings['announce']), TObject(PtrUInt(tr.Integers[
                                                                             'id'])));
              end;
...
begin
  ...
  try
    ...
    MessageDlg(Format(sBlocklistUpdateComplete, [res.Integers[('blocklist-size')]]), mtInformation,
    [mbOK], 0);
    res.Free;
  finally
    req.Free;
...
begin
  ...
  try
    ...
    if gTorrents.SelCount < 2 then
      s :=Format(sTorrentVerification, [UTF8Encode(widestring(gTorrents.Items[idxName, gTorrents.
...
procedure TMainForm.gTorrentsCellAttributes(Sender: TVarGrid; ACol, ARow, ADataCol: integer; AState:
                                            TGridDrawState;
      ...
      case ADataCol of
        ...
        idxSeeds:
                  if not VarIsNull(Sender.Items[idxSeedsTotal, ARow]) then
                    Text :=GetSeedsText(Sender.Items[idxSeeds, ARow], Sender.Items[idxSeedsTotal,
                           ARow]);
        idxPeers:
                  Text :=GetPeersText(Sender.Items[idxPeers, ARow], -1, Sender.Items[
                         idxLeechersTotal, ARow]);
        ...
        idxAddedOn, idxCompletedOn, idxLastActive:
                                                   Text :=TorrentDateTimeToString(Sender.Items[
                                                          ADataCol, ARow],FFromNow);
        idxPriority:
    ...
    begin
      ...
      try
        ...
        with res.Arrays['torrents'].Objects[0] do
          n :=IncludeProperTrailingPathDelimiter(UTF8Encode(Strings['downloadDir'])) + UTF8Encode(
              widestring(gTorrents.Items[idxName, gTorrents.Row]));
...
procedure TMainForm.gTorrentsDrawCell(Sender: TVarGrid; ACol, ARow, ADataCol: integer; AState:
                                      TGridDrawState; const R: TRect; var ADefaultDrawing: boolean);
begin
  if ARow < 0 then exit;
...
begin
  if RenameTorrent(gTorrents.Items[idxTorrentId, ARow], UTF8Encode(widestring(gTorrents.Items[
     idxName, ARow])), Trim(Value)) then
    begin
          ...
          try
            ...
            for i:=ARow + 1 to FFiles.Count - 1 do
              if FFilesTree.RowLevel[i] > lvl then
                FFiles[idxFileFullPath, i] :=UTF8Decode(p + Copy(UTF8Encode(widestring(FFiles[
                                             idxFileFullPath, i])), len + 1, MaxInt))
              else
                break;
          finally
            FFiles.EndUpdate;
...
procedure TMainForm.lvFilterCellAttributes(Sender: TVarGrid; ACol, ARow, ADataCol: integer; AState:
                                           TGridDrawState; var CellAttribs: TCellAttributes);
begin
  if ARow < 0 then exit;
  with CellAttribs do
...
procedure TMainForm.lvFilterDrawCell(Sender: TVarGrid; ACol, ARow, ADataCol: integer; AState:
                                     TGridDrawState; const R: TRect;
...
procedure TMainForm.lvPeersCellAttributes(Sender: TVarGrid; ACol, ARow, ADataCol: integer; AState:
                                          TGridDrawState; var CellAttribs: TCellAttributes);
...
procedure TMainForm.lvTrackersCellAttributes(Sender: TVarGrid; ACol, ARow, ADataCol: integer; AState
                                             : TGridDrawState; var CellAttribs: TCellAttributes);
  ...
  try
    if not FStarted then
      begin
        ...
        if i + Ini.ReadInteger('Interface', 'CheckNewVersionDays', 5) <= Trunc(Now) then
          begin
            if Ini.ReadBool('Interface', 'AskCheckNewVersion', True) then
              begin
                ...
                if not Ini.ReadBool('Interface', 'CheckNewVersion', False) then
                  if MessageDlg(Format(SCheckNewVersion, [AppName]), mtConfirmation, mbYesNo, 0) =
                     mrYes then
    ...
    if RpcObj.Connected then
      ...
      begin
        txReconnectSecs.Caption :=Format(sReconnect, [FReconnectTimeOut - Round(SecsPerDay*(Now -
                                  FReconnectWaitStart))]);
    ...
    if FDetailsWait.Visible then
      ...
      if (FDetailsWaitStart <> 0) and (Now - FDetailsWaitStart >= 300/MSecsPerDay) then
        begin
          CenterDetailsWait;
...
RpcObj.Url :=Format('%s://%s:%d', [RpcObj.Url, Ini.ReadString(Sec, 'Host', ''), Ini.ReadInteger(Sec,
             'Port', 9091)]);
SetRefreshInterval;
      ...
      begin
        ...
        if j > 0 then
          begin
            ...
            if (CompareFilePath(ss, fn) = 0) or (CompareFilePath(sss, Copy(fn, 1, Length(sss))) = 0)
              then
              begin
                Result :=Copy(s, 1, j - 1);
                d :='/';
...
procedure TMainForm.ConnectionSettingsChanged(const ActiveConnection: string; ForceReconnect:
                                              boolean);
...
begin
  ...
  acOpenFile.Enabled :=acSetHighPriority.Enabled and (lvFiles.SelCount < 2) and (RpcObj.RPCVersion >
                       = 4);
  ...
  try
    ...
    for i:=0 to list.Count - 1 do
      begin
        ...
        j :=t.Integers['status'];
        if ExistingRow and (j = TR_STATUS_SEED) and (FTorrents[idxStatus, row] = TR_STATUS_DOWNLOAD)
          then
            ...
            if j > 0 then
              begin
                ...
                if AnsiCompareText(ss, 'bt') = 0 then
                  ...
                  if (Length(ss) = 3) and (AnsiCompareText(Copy(ss, 1, 2), 'bt') = 0) and (ss[3] in
                     ['1'..'9']) then
                    System.Delete(s, 1, 4);
              end;
            j :=Pos(':', s);
        ...
        if FieldExists[idxPath] then
          FTorrents[idxPath, row] :=UTF8Decode(ExcludeTrailingPathDelimiter(UTF8Encode(t.Strings[
                                    'downloadDir'])))
        else
    ...
    try
      ...
      for i:=0 to FTorrents.Count - 1 do
        begin
          ...
          if (j = TR_STATUS_CHECK) or (j = TR_STATUS_CHECK_WAIT) or (j = TR_STATUS_DOWNLOAD_WAIT)
            then
            inc(WaitingCnt);
          ...
          if (PathFilter <> '') and not VarIsEmpty(FTorrents[idxPath, i]) and (UTF8Decode(PathFilter
             ) <> FTorrents[idxPath, i]) then
            continue;
          ...
          case FilterIdx of
            fltActive:
                       if not IsActive then
                         continue;
            fltInactive:
                         if (IsActive=true) or ((StateImg in [imgStopped, imgDone])=true) then
                           // PETROV
                           continue;
            fltDown:
            ...
            fltError:
                      if not (StateImg in [imgDownError, imgSeedError, imgError]) then
                        continue;
            fltWaiting:
                        if (FTorrents[idxStatus, i] <> TR_STATUS_CHECK) and (FTorrents[idxStatus, i]
                           <> TR_STATUS_CHECK_WAIT) and (FTorrents[idxStatus, i] <>
                           TR_STATUS_DOWNLOAD_WAIT)then
                          continue;
          end;

          if edSearch.Text <> '' then
            if UTF8Pos(UTF8UpperCase(edSearch.Text), UTF8UpperCase(UTF8Encode(widestring(FTorrents[
               idxName, i])))) = 0 then
              continue;
      ...
      for i:=0 to Paths.Count - 1 do
        begin
          ...
          for row:=StatusFiltersCount + 1 to j - 1 do
            if ExtractFileName(UTF8Encode(widestring(lvFilter.Items[-1, row]))) = s then
              begin
                s :=Paths[i];
                lvFilter.Items[0, row] :=UTF8Decode(Format('%s (%d)', [UTF8Encode(widestring(
                                         lvFilter.Items[-1, row])), ptruint(Paths.Objects[row -
...
TrayIcon.Hint :=Format(sDownloadingSeeding,
                [RpcObj.InfoStatus, LineEnding, DownCnt, SeedCnt, LineEnding, StatusBar.Panels[1].
                Text, StatusBar.Panels[2].Text]);
{$endif LCLcarbon}
finally
  Paths.Free;
end;
...
begin
  ...
  try
    ...
    ids :=GetSelectedTorrents;
    if gTorrents.SelCount < 2 then
      s :=Format(Msg, [UTF8Encode(widestring(gTorrents.Items[idxName, gTorrents.Items.IndexOf(
          idxTorrentId, ids[0])]))])
    else
...
procedure TMainForm.FillFilesList(ATorrentId: integer; list, priorities, wanted: TJSONArray; const
                                  DownloadDir: WideString);
begin
...
begin
  ...
  if t.IndexOfName('secondsDownloading') >= 0 then
    begin
      ...
      if f > 0 then
        s :=Format('%s (%s: %s)', [s, SAverage, GetHumanSize(t.Floats['downloadedEver']/f, 1) +
            sPerSecond]);
    end;
  ...
  if gTorrents.Items[idxSize, idx] >= 0 then
    begin
      txTotalSize.Caption :=Format(sDone, [GetHumanSize(t.Floats['totalSize']), GetHumanSize(t.
                            Floats['sizeWhenDone'] - t.Floats['leftUntilDone'])]);
      ...
      txPieces.Caption :=Format(sHave, [t.Integers['pieceCount'], GetHumanSize(t.Floats['pieceSize']
                         ), i]);
    end
  else
    begin
      ...
      begin
        ...
        if NoInfo then
          ...
          begin
            if i = 0 then
              begin
                lvTrackers.Items[idxTrackersListStatus, row] :=gTorrents.Items[idxTrackerStatus,
                                                               tidx];
      ...
      if s <> '' then
        begin
          ...
          if Fatal then
            DoDisconnect;
          ForceAppNormal;
          if Fatal and not RpcObj.Connected and RpcObj.ReconnectAllowed and (FReconnectTimeOut <> -1
             ) then
            begin
...
function TMainForm.TorrentAction(const TorrentIds: variant; const AAction: string; args: TJSONObject
): boolean;
...
function TMainForm.SetFilePriority(TorrentId: integer; const Files: array of integer; const
...
function TMainForm.ExecRemoteFile(const FileName: string; SelectFile: boolean; Userdef: boolean):
                                                                                             boolean
;
            ...
            begin
              ...
              for i := 0 to lvFiles.Items.Count-1 do
                if lvFiles.RowSelected[i] then
                  if r = '' then r := MapRemoteToLocal(FFilesTree.GetFullPath(i)) + '"'
                else
  ...
  try
    if gTorrents.Items.Count > 0 then
      begin
        ...
        StatusBar.Panels[7].Text :=Format(sTotalRemain,[GetHumanSize(TotalSizeToDownload -
                                   TotalDownloaded, 0, '?')]);
      end
    else
      begin
    ...
    try
      ...
      if EditMode then
        begin
          ...
          edTracker.Text :=UTF8Encode(widestring(lvTrackers.Items[idxTrackersListName, lvTrackers.
                           Row]));
          id :=lvTrackers.Items[idxTrackerID, lvTrackers.Row];
        end;
      AppNormal;
      if ShowModal = mrOk then
        begin
          ...
          try
            ...
            args.Add('ids', TJSONArray.Create([torid]));
            if EditMode then
              args.Add('trackerReplace', TJSONArray.Create([id, UTF8Encode(edTracker.Text)]))
              //fix bag
            else
              args.Add('trackerAdd', TJSONArray.Create([UTF8Encode(edTracker.Text)]));
            //fix bag
            req.Add('arguments', args);
            args :=nil;
            args :=RpcObj.SendRequest(req, False);
          ...
          if (ss = fn) or (Pos(IncludeProperTrailingPathDelimiter(ss), fn) = 1) then
            begin
              if ss = fn then
                ...
                begin
                  ss :=IncludeProperTrailingPathDelimiter(ss);
                  ss :=IncludeTrailingPathDelimiter(Copy(s, j + 1, MaxInt)) + Copy(fn, Length(ss) +
                       1, MaxInt);
                end;
...
begin
  try
    ...
    if not IsProtocolSupported(s) then
      exit;
    if (Pos('magnet:', LazUTF8.UTF8LowerCase(s)) <> 1) and (LazUTF8.UTF8LowerCase(Copy(s, Length(s)
       - Length(strTorrentExt) + 1, MaxInt)) <> strTorrentExt) then
      exit;
...
procedure _FillMenu(Items: TMenuItem; const Speeds: string; OnClickHandler: TNotifyEvent; CurSpeed:
                    integer);

...
type
  TVarGrid = class;
    ...
    TOnCellAttributes = procedure (Sender: TVarGrid; ACol, ARow, ADataCol: integer; AState:
                                   TGridDrawState; var CellAttribs: TCellAttributes) of object;
    TOnDrawCellEvent = procedure (Sender: TVarGrid; ACol, ARow, ADataCol: integer; AState:
                                  TGridDrawState; const R: TRect; var ADefaultDrawing: boolean) of
                       object;
    ...
    TOnQuickSearch = procedure (Sender: TVarGrid; var SearchText: string; var ARow: integer) of
                     object;
  ...
  { TVarGrid }
    ...
    TVarGrid = class(TCustomDrawGrid)
      ...
      protected
        ...
        override;
        procedure DoOnCellAttributes(ACol, ARow, ADataCol: integer; AState: TGridDrawState; var
        ...
        procedure Click;
        override;
        procedure GetCheckBoxState(const aCol, aRow:Integer; var aState:TCheckboxState);
        override;
        procedure SetCheckboxState(const aCol, aRow:Integer; const aState: TCheckboxState);
        override;
        procedure SetupCell(ACol, ARow: integer; AState: TGridDrawState; out CellAttribs:
                            TCellAttributes);
      ...
      published
        ...
        property OnTreeButtonClick: TCellNotifyEvent read FOnTreeButtonClick write
                                    FOnTreeButtonClick;
        ...
        begin
          if CellNeedsCheckboxBitmaps(aCol,aRow) then
            ...
            begin
              ...
              if (ca.ImageIndex <> -1) and Assigned(FImages) then
                begin
                  FImages.Draw(Canvas, aRect.Left + 2, (aRect.Bottom + aRect.Top - FImages.Height)
                  div 2, ca.ImageIndex, gdeNormal);
                    ...
                    begin
                      ...
                      if (Left <> Right) and (Top <> Bottom) then
                        begin
                          ...
                          ExtUTF8Out(Canvas.Handle, Left, Top, ETO_OPAQUE or ETO_CLIPPED, @aRect,
                                     PChar(ca.Text), Length(ca.Text), nil);
                        end;
                    end;
...
{$endif LCLcarbon}
  ...
  if (ssRight in Shift) {$ifdef darwin} or (Shift*[ssLeft, ssCtrl] = [ssLeft, ssCtrl]) {$endif} then
    begin
      SetFocus;
    ...
    if MultiSelect and (ssLeft in Shift) and (pt.x >= FixedCols) and (pt.y >= FixedRows) then
      begin
        if IsCtrl then
          begin
            if SelCount = 0 then
...
begin
  ...
  pt :=MouseToCell(Point(x, y));
  if (FHintCell.x <> -1) and ((FHintCell.x <> pt.x) or (FHintCell.y <> pt.y)) then
    begin
...
begin
  ...
  if (Shift = []) and ( (k = VK_SPACE) or (k = VK_LEFT) or (k = VK_RIGHT) or (k = VK_ADD) or (k =
     VK_SUBTRACT) ) then
    begin
      ...
      case k of
        ...
        VK_LEFT, VK_SUBTRACT:
                              if (coDrawTreeButton in ca.Options) and ca.Expanded then
                                begin
        ...
        VK_RIGHT, VK_ADD:
                          if (coDrawTreeButton in ca.Options) and not ca.Expanded then
                            begin
                              DoOnTreeButtonClick(FixedCols, inherited Row);
                              Key :=0;
                              exit;
  ...
  if MultiSelect then
    begin
      if ssCtrl in Shift then
        ...
        if k in [VK_LEFT, VK_RIGHT, VK_UP, VK_DOWN, VK_HOME, VK_END, VK_NEXT, VK_PRIOR] then
          begin
            if SelCount > 0 then
              RemoveSelection;
            FAnchor :=-1;
...
procedure TVarGrid.DoOnCellAttributes(ACol, ARow, ADataCol: integer; AState: TGridDrawState; var
                                      CellAttribs: TCellAttributes);
begin
...
procedure TVarGrid.SetupCell(ACol, ARow: integer; AState: TGridDrawState; out CellAttribs:
                             TCellAttributes);
...
begin
  ...
  FixedRows :=1;
  FixedCols :=0;
  Options :=[goRowSelect, goThumbTracking, goVertLine, goHorzLine, goColSizing, goColMoving,
            goDblClickAutoSize, goFixedHorzLine, goFixedVertLine];
...
begin
  with GCache do
    begin
      if (aCol=HotCell.x) and (aRow=HotCell.y) and not ((PushedCell.X<>-1) and (PushedCell.Y<>-1))
        then
        begin
          Include(gds, gdHot);
          HotCellPainted :=True;
